...
class Anggota extends CI_Controller {
    ...
    {	
        ...
        $this->load->view('anggota/anggota_view',$this->data);
    ...
    {	
        $this->data['idbo'] = $this->session->userdata('ses_id');
        $this->data['anggota'] = $this->M_Admin->get_table('tbl_login');
        
        $this->data['title_web'] = 'Tambah anggota ';
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
        $this->load->view('anggota/tambah_view',$this->data);
        $this->load->view('footer_view',$this->data);
    ...
    public function add()
    {
		...
		$id = $this->M_Admin->buat_kode('tbl_anggota','AG','id_login','ORDER BY id_login DESC LIMIT 1'); 
		...
		$dd = $this->db->query("SELECT * FROM tbl_anggota WHERE user = '$user' OR email = '$email'");
		if($dd->num_rows() > 0)
		{
			$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-warning">
			<p> Gagal Update User : '.$user.' !, Username / Email Anda Sudah Terpakai</p>
			</div></div>');
    ...
    {	
		if($this->session->userdata('level') == 'Petugas'){
			if($this->uri->segment('3') == ''){ echo '<script>alert("halaman tidak ditemukan");window.location="'.base_url('anggota').'";</script>';}
			$this->data['idbo'] = $this->session->userdata('ses_id');
			$count = $this->M_Admin->CountTableId('tbl_anggota','id_login',$this->uri->segment('3'));
			if($count > 0)
			{			
				$this->data['anggota'] = $this->M_Admin->get_tableid_edit('tbl_anggota','id_login',$this->uri->segment('3'));
			}else{
				echo '<script>alert("anggota TIDAK DITEMUKAN");window.location="'.base_url('anggota').'"</script>';
			}
			...
			if($count > 0)
			{			
				$this->data['anggota'] = $this->M_Admin->get_tableid_edit('tbl_anggota','id_login',$this->session->userdata('ses_id'));
			}else{
				echo '<script>alert("anggota TIDAK DITEMUKAN");window.location="'.base_url('anggota').'"</script>';
			}
		}
        ...
        $this->load->view('header_view',$this->data);
        $this->load->view('sidebar_view',$this->data);
        $this->load->view('anggota/edit_view',$this->data);
        $this->load->view('footer_view',$this->data);
	}
    ...
    {	
		if($this->session->userdata('level') == 'Petugas'){
			...
			$this->data['idbo'] = $this->session->userdata('ses_id');
			$count = $this->M_Admin->CountTableId('tbl_anggota','id_login',$this->session->userdata('ses_id'));
			if($count > 0)
			{			
				$this->data['anggota'] = $this->M_Admin->get_tableid_edit('tbl_anggota','id_login',$this->session->userdata('ses_id'));
			}else{
